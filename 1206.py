...
for tc in range(1,11):
    ...
    res = 0 # 조망권이 확보된 세대 수를 저장하는 변수 초기화

    for i in range(2,tm-2): #맨왼쪽 2개와 맨오른쪽 2개를 제외하고 건물을 반복
        # 양옆으로 2칸씩 비워져야하니까 현재건물이 왼쪽2칸과 오른쪽2칸보다 높아야됨
        if build[i] > build[i-2] and build[i] > build[i-1] and build[i] > build[i+1] and build[i] > build[i+2]:
            max_build = build[i-2]#비교하는 4개중에 가장 높은건물 구하기 위해 변수 초기화

            for j in range(-2,3):# 왼쪽 2, 1칸 오른쪽 1, 2칸의 건물을 순회
                if j == 0: # j가 0인 경우는 자기 자신이기 때문에 넘어간다.
                    continue
                if build[i-j] > max_build:# 4칸의 건물 중 가장 높은 건물의 높이를 구한다
                    max_build = build[i-j]
            # 기준이 되는 건물의 높이 - 비교 건물 중 가장 큰 건물의 높이 = 조망권 확보 세대. 이 값을 res 변수에 더해준다.
            res += build[i] - max_build
    print(f'#{tc} {res}')