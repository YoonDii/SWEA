...
for x in range(1,t+1): #1~t까지
    p,q,r,s,w = map(int,input().split()) # 테스트 케이스 정수입력하기

    ac = p * w # 1L당p요금 x 민수사용값 w
    bc = 0   # bc는 기본요금q 와 r초과유무에 따른 추가요금 s //일단 bc값 초기화
    if w <= r : # w가 r이하일때 bc는 q기본요금
        bc = q
    else: # w가 r초과시 초과한 L만큼 s요금부과
        bc = q + (w-r)*s
    if ac < bc: # a회사가 b회사보다 요금이 적으면 a회사 출력
        print('#{} {}'.format(x,ac))
    else: # b회사가 a회사보다 요금이 적으면 b회사 출력
        print('#{} {}'.format(x,bc))
...
for x in range(1,t+1): 
    ...
    #print(p,q,r,s,w) # 이거 print로 맞게 나오는지 먼저 확인하기
    ac = p * w
    if w <= r :
        bc = q
    else: 
